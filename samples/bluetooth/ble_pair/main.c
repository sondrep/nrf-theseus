...
static const char *device_name = "MyNewt Device";
...
static void ble_host_task(void *param)
{
	...
	LOG("[APP] ble_host_task: init start\n");
	...
	LOG("[APP] ble_svc_gap_device_name_set rc=%d\n", rc);
	assert(rc == 0);

	LOG("[APP] ble_host_task: entering nimble_port_run\n");
	nimble_port_run();
...
int main(void)
{
	...
	LOG("[APP] xTaskCreate(ble_hs) ok=%ld\n", (long)ok);
	assert(ok == pdPASS);
