...
#define IDENTIFY_LED BOARD_PIN_LED_3

/* Button which reopens the Zigbee Network. */
#define KEY_ZIGBEE_NETWORK_REOPEN BOARD_PIN_BTN_0

/* Button used to enter the Identify mode. */
#define IDENTIFY_MODE_BUTTON BOARD_PIN_BTN_1
...
ZB_DECLARE_RANGE_EXTENDER_CLUSTER_LIST(nwk_coordinator_clusters, basic_attr_list,
				       identify_attr_list);

ZB_DECLARE_RANGE_EXTENDER_EP(nwk_coordinator_ep, ZIGBEE_COORDINATOR_ENDPOINT,
...
static void toggle_identify_led(zb_bufid_t bufid)
{
	...
	if ((++blink_status) % 2) {
		nrfx_gpiote_out_set(gpiote, IDENTIFY_LED);
	} else {
		nrfx_gpiote_out_clear(gpiote, IDENTIFY_LED);
	}
...
static void start_identifying(zb_bufid_t bufid)
{
	...
	if (ZB_JOINED()) {
		...
		if (dev_ctx.identify_attr.identify_time ==
			...
			zb_ret_t zb_err_code =
				zb_bdb_finding_binding_target(ZIGBEE_COORDINATOR_ENDPOINT);
...
static void steering_finished(zb_uint8_t param)
{
	...
	LOG("Network steering finished\n");
	nrfx_gpiote_t *gpiote = theseus_gpiote_get();
	nrfx_gpiote_out_clear(gpiote, ZIGBEE_NETWORK_STATE_LED);
}
 ...
 */
static void button_changed(nrfx_gpiote_pin_t pin, nrfx_gpiote_trigger_t trigger, void *p_context)
{
	/* Calculate bitmask of buttons that are pressed and have changed their state. */
	zb_bool_t comm_status;

	if (pin == KEY_ZIGBEE_NETWORK_REOPEN) {
		(void)(ZB_SCHEDULE_APP_ALARM_CANCEL(steering_finished, ZB_ALARM_ANY_PARAM));

		comm_status = bdb_start_top_level_commissioning(ZB_BDB_NETWORK_STEERING);
		if (comm_status) {
			LOG("Top level commissioning restated\n");
		} else {
			LOG("Top level commissioning hasn't finished yet!\n");
		}
	}

	if (pin == IDENTIFY_MODE_BUTTON) {
...
void zboss_signal_handler(zb_bufid_t bufid)
{
	...
	case ZB_BDB_SIGNAL_DEVICE_REBOOT:
		...
		if (status == RET_OK) {
			if (ZIGBEE_MANUAL_STEERING == ZB_FALSE) {
				...
				if (comm_status == false) {
					LOG("bdb_start_top_level_commissioning failed");
				}
			} else {
				LOG("Coordinator restarted successfully\n");
			}
		} else {
			LOG("Failed to initialize Zigbee stack using NVRAM data (status: %d)\n",
			    status);
		}
		break;
	...
	case ZB_BDB_SIGNAL_STEERING:
		if (status == RET_OK) {
			if (ZIGBEE_PERMIT_LEGACY_DEVICES == ZB_TRUE) {
				LOG("Allow pre-Zigbee 3.0 devices to join the network\n");
				zb_bdb_set_legacy_device_support(1);
			}

			/* Schedule an alarm to notify about the end of steering period.
			 */
			LOG("Network steering started\n");
			zb_err_code = ZB_SCHEDULE_APP_ALARM(
				steering_finished, 0,
	...
	default:
		...
		zb_err_code = zigbee_default_signal_handler(bufid);
		if (zb_err_code != RET_OK) {
			LOG("zigbee_default_signal_handler failed, err = %d\n", zb_err_code);
		}
		break;
	}
	...
	if (ZB_JOINED() && (ZB_SCHEDULE_GET_ALARM_TIME(steering_finished, ZB_ALARM_ANY_PARAM,
						       &timeout_bi) == RET_OK)) {
		nrfx_gpiote_out_set(gpiote_instance, ZIGBEE_NETWORK_STATE_LED);
	} else {
		nrfx_gpiote_out_clear(gpiote_instance, ZIGBEE_NETWORK_STATE_LED);
	}
...
int main(void)
{
	...
	LOG("Theseus: Starting Zigbee R23 Coordinator example\n");
	...
	nrfx_gpiote_output_configure(gpiote, ZIGBEE_NETWORK_STATE_LED, &gpiote_output_cfg, NULL);
	nrfx_gpiote_output_configure(gpiote, IDENTIFY_LED, &gpiote_output_cfg, NULL);

	nrfx_gpiote_trigger_config_t trigger_cfg = {.trigger = NRFX_GPIOTE_TRIGGER_LOTOHI,
						    .p_in_channel = NULL};
	nrfx_gpiote_handler_config_t handler_cfg = {.handler = button_changed, .p_context = NULL};
	...
	nrfx_gpiote_input_configure(gpiote, KEY_ZIGBEE_NETWORK_REOPEN, &btn_cfg);
	nrfx_gpiote_input_configure(gpiote, IDENTIFY_MODE_BUTTON, &btn_cfg);

	nrfx_gpiote_trigger_enable(gpiote, KEY_ZIGBEE_NETWORK_REOPEN, true);
	...
	LOG("Zigbee R23 Coordinator example started\n");
