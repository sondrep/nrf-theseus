...
#define HT_DT_HAS_RESERVED_DPPIC_CHANNEL                                                           \
	COND_CODE_1(DT_NODE_HAS_PROP(DT_NODELABEL(dppic130), sink_channels),                       \
		    (DT_FOREACH_PROP_ELEM(DT_NODELABEL(dppic130), sink_channels,                   \
					  _HT_DT_CHECK_DPPIC_CHANNEL) false),                      \
		    (false))

BUILD_ASSERT(HT_DT_HAS_RESERVED_DPPIC_CHANNEL, "The required DPPIC_G1 channel is not reserved");
...
#define HT_DT_HAS_RESERVED_DPPIC_G2_CHANNEL                                                        \
	COND_CODE_1(DT_NODE_HAS_PROP(DT_NODELABEL(dppic132), source_channels),                     \
		    (DT_FOREACH_PROP_ELEM(DT_NODELABEL(dppic132), source_channels,                 \
					  _HT_DT_CHECK_DPPIC_G2_CHANNEL) false),                   \
		    (false))

BUILD_ASSERT(HT_DT_HAS_RESERVED_DPPIC_G2_CHANNEL, "The required DPPIC_G2 channel is not reserved");
...
#define HT_DT_HAS_RESERVED_IPCT_L_LINK                                                             \
	COND_CODE_1(DT_NODE_HAS_PROP(DT_NODELABEL(cpurad_ipct), sink_channel_links),               \
		    (DT_FOREACH_PROP_ELEM(DT_NODELABEL(cpurad_ipct), sink_channel_links,           \
 ...
 */
BUILD_ASSERT(HT_DT_HAS_RESERVED_IPCT_L_LINK, "The required IPCT_L link is not reserved");
...
#define TS_DT_HAS_RESERVED_IPCT_G1_LINK                                                            \
	COND_CODE_1(DT_NODE_HAS_PROP(DT_NODELABEL(ipct130), source_channel_links),                 \
		    (DT_FOREACH_PROP_ELEM(DT_NODELABEL(ipct130), source_channel_links,             \
 ...
 * array of triplets that is difficult to separate using macros.
 */
BUILD_ASSERT(TS_DT_HAS_RESERVED_IPCT_G1_LINK, "The required IPCT_G1 link is not reserved");
...
void nrf_802154_platform_sl_lptimer_hw_task_cross_domain_connections_setup(uint32_t cc_channel)
{
	...
	uint32_t eep = nrfx_grtc_event_compare_address_get(cc_channel);
	int err;
