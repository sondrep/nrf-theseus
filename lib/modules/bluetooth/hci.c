 ...
 * Doing it here, once, before mpsl_init() keeps the BLE samples from having to touch the GRTC
 * themselves. */
static void grtc_lfclk_init(void)
{
	uint8_t grtc_channel;

	/* The GRTC is shared: MPSL uses it for radio timing and the FreeRTOS tick
	 * (vPortSetupTimerInterrupt() in port.c) uses it as the kernel tick. Only one
	 * nrfx_grtc_init() is allowed; a second returns -EALREADY.
	 *
	 ...
	 * assert() halts loudly so a broken clock is caught here rather than failing silently later
	 * inside MPSL. */
...
/* Brings up the whole link layer:
 * the clocks, the MPSL (radio arbitration), and the SoftDevice Controller.
 *
 * NimBLE calls this once during ble_transport_init(),
 ...
 * Same idea as the ISO path:
 * NimBLE gives us an mbuf chain, the SDC wants one flat buffer,
...
/* Handle an HCI command coming down from the host.
 * We decode the opcode (OGF = command group, OCF = command within it),
 * call the matching SDC function, and build the command-complete event the host expects back.
 *
 * The buffer is reused in place:
 * it comes in holding the command and goes out holding the response event,
 * so we write the response payload after the event header and fill in the header at the end.
...
int ble_transport_to_ll_cmd_impl(void *buf)
{
	...
	case 0x04:
		...
		case BLE_HCI_OCF_IP_RD_LOCAL_VER:
			...
			hci_ev->length =
				sizeof(sdc_hci_cmd_ip_read_local_version_information_return_t);
			break;

		case BLE_HCI_OCF_IP_RD_LOC_SUPP_CMD:
			/* This SDC variant does not export
			 * sdc_hci_cmd_ip_read_local_supported_commands(), so return an all-zero (no
			 * optional commands) map. The host needs the full 64-byte response, a short
			 * reply triggers a host reset that tears advertising back down. */
			memset(rspbuf, 0,
		...
		case BLE_HCI_OCF_IP_RD_LOC_SUPP_FEAT:
			...
			hci_ev->length =
				sizeof(sdc_hci_cmd_ip_read_local_supported_features_return_t);
			break;
	...
	case 0x08:
		...
		case BLE_HCI_OCF_LE_RD_LOC_SUPP_FEAT:
			...
			hci_ev->length =
				sizeof(sdc_hci_cmd_le_read_local_supported_features_return_t);
			break;
		...
		case BLE_HCI_OCF_LE_SET_CONN_CTE_TX_PARAMS:
			...
			hci_ev->length =
				sizeof(sdc_hci_cmd_le_set_conn_cte_transmit_params_return_t);
			break;
