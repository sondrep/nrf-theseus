...
static inline size_t chunk_usable(chunk_t *c)
{
 ...
 * pool */
static inline void make_free_chunk(chunk_t *c, size_t size)
{
	c->size = size;
	__malloc_free(chunk_to_ptr(c));
}
/* List list header of free blocks */
static chunk_t *__malloc_free_list;

/* Starting point of memory allocated from system */
static char *__malloc_sbrk_start;
static char *__malloc_sbrk_top;
 ...
 *   need it
 */
static void *__malloc_sbrk_aligned(size_t s)
{
	char *p, *align_p;
...
static bool __malloc_grow_chunk(chunk_t *c, size_t new_size)
{
 ...
 */
static void *shmem_malloc(size_t s)
{
	chunk_t **p, *r;
	char *ptr;
	size_t alloc_size;
 ...
 */
static void shmem_free(void *free_p)
{
	chunk_t *p_to_free;
	chunk_t **p, *r;
...
static void *calloc(size_t n, size_t elem)
{
	size_t bytes;
...
static void __malloc_validate_block(chunk_t *r)
{
	__malloc_block = r;
...
static void __malloc_validate(void)
{
	chunk_t *r;
 ...
 *            in the padding area.
 */
static void *memalign(size_t align, size_t s)
{
	chunk_t *chunk_p;
...
static int mallopt(int parameter_number, int parameter_value)
{
