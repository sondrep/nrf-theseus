...
#include <string.h>
#include <errno.h>
#include <theseus/nfc/ndef/record.h>
			   ...
			   uint8_t *record_buffer, uint32_t *record_len)
{
	...
	if (record_buffer) {
		...
		payload_len[0] = (uint8_t)(record_payload_len >> 24);
		payload_len[1] = (uint8_t)(record_payload_len >> 16);
		payload_len[2] = (uint8_t)(record_payload_len >> 8);
		payload_len[3] = (uint8_t)(record_payload_len);
	}
